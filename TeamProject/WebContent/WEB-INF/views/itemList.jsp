...
<body>
	<%
				...
				List<ItemVO> list = iDao.selectAllItems(startRow, endRow);
	%>
	<div id="wrap" style="width: 900px" align="center">
		...
		<%
			...
			if (startPage > 10) {
		%>
		<a href="itemList.do?pageNum=<%=startPage - 10 %>&pageSize=<%=pageSize%>">[이전]</a>
		<%
			}
			for (int i = startPage; i <= endPage; i++) {
		%>
		<a href="itemList.do?pageNum=<%=i %>&pageSize=<%=pageSize%>">[<%=i %>]</a>
		<%
			}
			if (endPage < pageCount) {
		%>
		<a href="itemList.do?pageNum=<%=startPage + 10 %>&pageSize=<%=pageSize%>">[다음]</a>
		<%
			}
		%>
		<br><br>
