...
public class RecentAddrDAO {

	private static RecentAddrDAO instance = new RecentAddrDAO();
	
	public static RecentAddrDAO getInstance() {
		return instance;
	}
	...
	public void insertAddr (RecentAddrDTO adto) {
		String sql = "insert into recentaddrtbl values(deli_addr_seq.nextVal,?,?,?,?,?,?)";
	...
	public ArrayList<RecentAddrDTO> selectAddrList(String userid) {
		String sql = "select deli_addr_seq, deli_name, deli_addr, deli_phone, deli_msg, deli_pwd from recentaddrtbl where userid=? order by deli_addr_seq asc";
		ArrayList<RecentAddrDTO> addrList = new ArrayList<RecentAddrDTO>();
		Connection conn = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
	...
	public int addrCheck(String deli_postcode, String deli_name, String userid) {
		...
		String sql = "select count(*) from recentaddrtbl where (deli_addr like ? and deli_name like ?) and userid=?";
	...
	public void deleteAddr(int deli_addr_seq) {
		String sql = "delete from recentaddrtbl where deli_addr_seq=?";
		Connection conn = null;
		PreparedStatement pstmt = null;
	...
	public int addrCnt(String userid) {
		int addrCnt = 0;
		String sql = "select count(*) from recentaddrtbl where userid=?";
	...
	public int oldAddrFind(String userid) {
		int oldAddrSeq = 0;
		String sql = "select deli_addr_seq from (select deli_addr_seq from recentaddrtbl where userid=? order by deli_addr_seq asc) where rownum=1";
